-- ============================================================================
-- ZINTRA CREDITS SYSTEM - DATABASE MIGRATION (FIXED)
-- Date: 30 January 2026
-- Purpose: Add credit-based payment system tables to Supabase
-- Status: ✅ Ready to execute - No schema dependencies
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS public.credits_packages (
  ...
  CONSTRAINT unique_package_per_type_name UNIQUE(user_type, package_name)
);
...
CREATE INDEX idx_credit_transactions_mpesa_id ON public.credit_transactions(mpesa_request_id);
...
INSERT INTO public.credits_packages (user_type, package_name, credit_amount, price_ksh, discount_percentage, features, position) VALUES
...
ON CONFLICT (user_type, package_name) DO NOTHING;
...
-- 10. CREATE FUNCTION to deduct credits (atomic operation)
...
-- SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename LIKE 'credit%';
-- Expected: 6 tables

-- Verify functions created:
-- SELECT proname FROM pg_proc WHERE pronamespace = 'public'::regnamespace AND proname IN ('deduct_user_credits', 'add_user_credits');
