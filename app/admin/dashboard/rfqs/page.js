...
import {
  ADMIN_ACTIVE_RFQ_STATUSES,
  ADMIN_PENDING_RFQ_STATUSES,
  CLOSED_RFQ_STATUSES,
  normalizeRfqRecord,
...
const pendingStatuses = ADMIN_PENDING_RFQ_STATUSES;
const activeStatuses = ADMIN_ACTIVE_RFQ_STATUSES;
...
export default function ConsolidatedRFQs() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const normalizedRfqs = (allRfqs || []).map(normalizeRfqRecord);
      setRfqs(normalizedRfqs);

      // Calculate stats
      const pendingApproval = normalizedRfqs.filter(r => 
        pendingStatuses.includes(r.status)
      ).length;
      
      const activeRFQs = normalizedRfqs.filter(r => 
        activeStatuses.includes(r.status)
      ).length;
      
      const closedRFQs = normalizedRfqs.filter(r => 
        CLOSED_RFQ_STATUSES.includes(r.status)
      ).length;
      ...
      const avgResponseRate = normalizedRfqs.length > 0
        ? (totalResponses / normalizedRfqs.length * 100).toFixed(1)
        : 0;

      // Count by RFQ type
      const directCount = normalizedRfqs.filter(r => r.type === 'direct').length;
      const matchedCount = normalizedRfqs.filter(r => ['matched', 'wizard'].includes(r.type)).length;
      const publicCount = normalizedRfqs.filter(r => r.type === 'public').length;

      // Calculate average match quality for matched RFQs
      const matchedRFQs = normalizedRfqs.filter(r => ['matched', 'wizard'].includes(r.type));
      ...
      const publicRFQs = normalizedRfqs.filter(r => r.type === 'public');
      ...
      setStats({
        ...
        matchedCount,
        publicCount,
        totalRFQs: normalizedRfqs.length || 0,
        averageMatchQuality,
      ...
      normalizedRfqs.forEach(rfq => {
        ...
        // Matched RFQ alerts
        if (['matched', 'wizard'].includes(rfq.type)) {
  ...
  const notifyVendors = async (rfq) => {
    const category = rfq.category_display || rfq.category || rfq.auto_category;
    ...
    let matching = (vendors || [])
      .filter((v) => {
        ...
        const categoryMatch = rfqMatchesVendorCategory(rfq, v) || categoryMatches(v.category, category);
                    ...
                    <div className="flex gap-3">
                      ...
                      {rfq.status === 'needs_admin_review' && (
                        <button
                          ...
                          className="px-4 py-2 border border-amber-300 rounded-lg text-amber-700 hover:bg-amber-50 transition"
                        >
                          Unsuccessful
                        </button>
                      )}
          ...
          <div className="bg-white rounded-lg max-w-lg w-full">
            <div className="border-b border-gray-200 p-6">
              ...
              <p className="text-sm text-gray-500 mt-1">
                This closes the RFQ and sends an explanation to the buyer.
              </p>
            </div>
            <div className="p-6 space-y-4">
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                <p className="font-semibold text-gray-900">{selectedRFQ.title}</p>
                <p className="text-sm text-gray-600 mt-1">
                  {selectedRFQ.category || 'Uncategorized'}
                  {selectedRFQ.county ? ` • ${selectedRFQ.county}` : ''}
                </p>
              </div>
              <div>
                ...
                >
                  ...
                  <option value="category_unavailable">No vendors available in this category</option>
                  <option value="location_unavailable">No vendors available in this location</option>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  placeholder="Explain what prevented this RFQ from moving forward..."
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-amber-500"
                />
              </div>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-sm text-blue-700">
                The buyer will be notified that this RFQ could not be fulfilled with the current vendor pool.
              </div>
            </div>
            <div className="border-t border-gray-200 p-6 flex gap-3">
              ...
              >
                {markingUnsuccessful ? 'Saving...' : 'Mark Unsuccessful'}
              </button>
            </div>
          </div>
        </div>
