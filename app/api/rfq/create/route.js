...
import { insertRfqRecipients, insertRfqRequests } from '@/lib/rfqPersistence';
import { getRfqBudgetDisplay } from '@/lib/rfqUtils';
...
export async function POST(request) {
  try {
    ...
    const budgetDisplay = getRfqBudgetDisplay({
    ...
    const rfqData = {
      ...
      budget_range: budgetDisplay !== 'Not specified' ? budgetDisplay : null,
      ...
      is_paid: false,
      visibility: 'private',
      timeline: sharedFields.desiredStartDate || null,
    ...
    if (rfqType === 'direct' && selectedVendors.length > 0) {
      try {
        ...
        const { error: requestError } = await insertRfqRequests(supabase, requestRecords);
        if (requestError) {
          console.error('[RFQ CREATE] rfq_requests compatibility insert error:', requestError);
        }
      } catch (err) {
    ...
    if (rfqType === 'wizard') {
      try {
        ...
        wizardNeedsAdminReview = !matchResult.success || matchResult.needsAdminReview;
        wizardMatchCount = matchResult.vendorCount || 0;

        if (wizardNeedsAdminReview) {
          await supabase
            .from('rfqs')
        ...
        console.log('[RFQ CREATE] Wizard RFQ - Matched', wizardMatchCount, 'vendors');
      } catch (err) {
        console.error('[RFQ CREATE] Vendor auto-match error:', err.message);
        wizardNeedsAdminReview = true;
        await supabase
          .from('rfqs')
          .update({ status: 'needs_admin_review' })
          .eq('id', rfqId);
    ...
    if (rfqType === 'vendor-request' && selectedVendors.length > 0) {
      try {
        ...
        const { error: requestError } = await insertRfqRequests(supabase, {
          rfq_id: rfqId,
          vendor_id: selectedVendors[0],
          user_id: userId,
          project_title: sharedFields.projectTitle?.trim() || 'Untitled RFQ',
        ...
        if (requestError) {
          console.error('[RFQ CREATE] Vendor-request rfq_requests insert error:', requestError);
        }
      } catch (err) {
        console.error('[RFQ CREATE] Vendor assignment error:', err.message);
      }
    ...
    // ============================================================================
    if (rfqType !== 'public' && !(rfqType === 'wizard' && wizardNeedsAdminReview)) {
    ...
    let successMessage = `RFQ created successfully! (${rfqType} type)`;
    let responseStatus = createdRfq.status;

    if (rfqType === 'wizard' && wizardNeedsAdminReview) {
      successMessage = 'Your RFQ has been submitted and needs admin review before vendors are matched.';
      responseStatus = 'needs_admin_review';
    } else if (rfqType === 'wizard') {
      successMessage = `RFQ auto-matched and sent to ${wizardMatchCount} vendor${wizardMatchCount === 1 ? '' : 's'}.`;
      responseStatus = 'submitted';
    } else if (rfqType === 'public') {
      successMessage = 'Your public RFQ has been submitted for admin approval before it is published to vendors.';
      responseStatus = 'pending_approval';
    } else if (rfqType === 'vendor-request') {
      successMessage = 'Your RFQ has been sent to the requested vendor.';
      responseStatus = 'submitted';
    } else if (rfqType === 'direct') {
      successMessage = 'Your RFQ has been sent directly to the selected vendor(s).';
      responseStatus = 'submitted';
    }
