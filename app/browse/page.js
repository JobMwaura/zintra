...
export default function BrowseVendors() {
  ...
  useEffect(() => {
    ...
    if (categoryParam) {
      ...
      const categoryObj = CANONICAL_CATEGORIES.find(
        cat => cat.slug === categoryParam || cat.label === categoryParam
      );
      if (categoryObj) {
        setSelectedCategory(categoryObj.label);
      } else {
        // Fallback: use the param as-is (might be a label)
  ...
  useEffect(() => {
    ...
    const fetchVendors = async () => {
      try {
        ...
        await Promise.race([
          (async () => {
            ...
            if (fetchError) {
              ...
              setVendors([]);
              // Set canonical categories even on error
              setCategories([
                'All Categories',
                ...CANONICAL_CATEGORIES.map((cat) => cat.label),
              ]);
            } else if (!vendorData || vendorData.length === 0) {
              console.warn('No vendors found in database');
              setVendors([]);
              // Set canonical categories for filtering even with no vendors
              setCategories([
                'All Categories',
                ...CANONICAL_CATEGORIES.map((cat) => cat.label),
              ]);
        ...
        setVendors([]);
        // Set canonical categories on error
        setCategories([
          'All Categories',
          ...CANONICAL_CATEGORIES.map((cat) => cat.label),
        ]);
  ...
  // ✅ Filtering logic with proper category slug matching
  const filteredVendors = vendors.filter((vendor) => {
    const matchesSearch =
      vendor.company_name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      vendor.description?.toLowerCase().includes(searchQuery.toLowerCase());
    
    // Convert label to slug for proper comparison
    ...
    if (!matchesCategory && selectedCategory) {
      ...
      if (selectedSlug) {
        ...
        if (vendor.primary_category_slug === selectedSlug) {
          matchesCategory = true;
        }
        // Check secondary categories match
