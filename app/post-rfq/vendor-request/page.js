...
/**
 * Vendor Request Quote Page
 * 
 * Entry Point: Vendor profile "Request Quote" button (or Direct RFQ page with vendor pre-selection)
 * Query Parameters:
 * - vendorId: The vendor ID to send the RFQ to
 * 
 * Key Characteristics:
 * - Category is LOCKED to vendor's primary category
 * - Vendor is PRE-SELECTED (recipient is the specified vendor only)
 * - No vendor selection step (vendor is fixed)
 * - RFQ is PRIVATE (only sent to this specific vendor)
 * - Modal opens automatically with vendor details
 */
function VendorRequestContent() {
