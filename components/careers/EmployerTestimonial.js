...
export default function EmployerTestimonial() {
  ...
  async function loadRedirectPaths() {
    try {
      console.log('[EmployerTestimonial] Starting to load redirect paths...');
      const jobPath = await getEmployerRedirectPath('job');
      const gigPath = await getEmployerRedirectPath('gig');
      console.log('[EmployerTestimonial] Loaded paths:', { jobPath, gigPath });
      setPostJobLink(jobPath);
      setPostGigLink(gigPath);
    } catch (error) {
      console.error('[EmployerTestimonial] Error loading redirect paths:', error);
    } finally {
      setLoading(false);
    }
  }
