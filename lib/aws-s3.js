...
const SIGNED_URL_EXPIRY = 3600; // 1 hour in seconds (for uploads)
const GET_URL_EXPIRY = 7 * 24 * 60 * 60; // 7 days in seconds (AWS SigV4 maximum - always generate fresh on page load)
...
export async function generatePresignedUploadUrl(
  ...
  try {
    ...
    const fileUrl = await getSignedUrl(s3Client, getCommand, {
      expiresIn: GET_URL_EXPIRY, // 365 days
    });
 ...
 * URLs are valid for 365 days to ensure they stay accessible
 * as long as the vendor profile is active
 *
 * @param {string} fileKey - The S3 object key
 * @param {number} expiresIn - Expiry time in seconds (default: 365 days)
 * @returns {string} Presigned URL
 */
export async function generateFileAccessUrl(fileKey, expiresIn = GET_URL_EXPIRY) {
  try {
    console.log('📝 Generating access URL for key:', fileKey);
    console.log('   Bucket:', BUCKET_NAME);
    ...
    console.error('❌ Error generating file access URL:', error.message);
