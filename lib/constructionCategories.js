...
/**
 * Normalize a category string to match our comprehensive list
 * Handles variations and aliases
 */
export function normalizeCategoryName(inputCategory) {
  if (!inputCategory) return null;
  
  const input = inputCategory.trim().toLowerCase();
  
  // Try exact match first
  const exactMatch = ALL_CATEGORIES_FLAT.find(
    cat => cat.label.toLowerCase() === input
  );
  if (exactMatch) return exactMatch.label;
  
  // Try fuzzy matching - check if the input contains key words from categories
  const fuzzyMatch = ALL_CATEGORIES_FLAT.find(cat => {
    const catLower = cat.label.toLowerCase();
    // Check if input is contained in category or category is in input
    return catLower.includes(input) || input.includes(catLower.split('&')[0].trim());
  });
 ...
 */
export function categoryMatches(vendorCategory, rfqCategory) {
  if (!vendorCategory || !rfqCategory) return false;
  
  const normalized_vendor = normalizeCategoryName(vendorCategory);
  const normalized_rfq = normalizeCategoryName(rfqCategory);
  
  // Exact match
  if (normalized_vendor === normalized_rfq) return true;
  
  // Partial match - if they share significant words
  const vendorWords = normalized_vendor.toLowerCase().split(/\s+/);
  const rfqWords = normalized_rfq.toLowerCase().split(/\s+/);
  
  const commonWords = vendorWords.filter(w => 
    rfqWords.includes(w) && w.length > 3
  );
...
/**
 * Get all vendors matching a category (including partial matches)
 */
export function filterVendorsByCategory(vendors, category) {
  if (!category || category === 'All Categories') return vendors;
  
  return vendors.filter(vendor => {
    if (vendor.category === category) return true;
    return categoryMatches(vendor.category, category);
  });
}
