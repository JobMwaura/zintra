 ...
 */
async function _flagForAdminReview(rfqId, categorySlug, county, reason = 'zero_matches') {
  ...
  try {
    ...
    if (admins?.length) {
      const adminNotifs = admins.map(a => ({
        ...
        body: `Auto-matching found 0 qualified vendors for category "${categorySlug}" in ${county || 'any location'}. Please manually assign vendors or mark as unsuccessful.`,
        metadata: { rfq_id: rfqId, category: categorySlug, county: county || null, reason: reason },
        related_id: rfqId,
        related_type: 'rfq',
      }));
...
/**
 * Internal: Flag a low-match RFQ for admin awareness (1-2 vendors matched)
 * Does NOT change RFQ status (vendors were already sent), but alerts admin
 * Admin can then: manually assign more vendors, or mark unsuccessful
 */
async function _flagLowMatchForAdmin(rfqId, categorySlug, county, matchCount) {
  try {
    const { data: admins } = await supabase
      .from('user_roles')
      .select('user_id')
    ...
    if (admins?.length) {
      const adminNotifs = admins.map(a => ({
        user_id: a.user_id,
        type: 'admin_rfq_low_match',
        title: '⚠️ Low Vendor Match Count',
        body: `Wizard RFQ matched only ${matchCount} vendor${matchCount !== 1 ? 's' : ''} for category "${categorySlug}" in ${county || 'any location'}. Consider assigning more vendors or contacting the buyer.`,
        metadata: {
          rfq_id: rfqId,
      ...
      await supabase.from('notifications').insert(adminNotifs);
      console.log('[AUTO-MATCH] ✅ Notified', admins.length, 'admin(s) about low match count');
    }
  } catch (e) {
    console.warn('[AUTO-MATCH] Low-match admin notification failed:', e.message);
  }
}
...
async function _scoreAndInsert(rfqId, candidates, rfqInfo) {
  ...
  if (qualified.length === 0) {
    ...
    return await _flagForAdminReview(rfqId, rfqInfo.categorySlug, rfqInfo.county, 'zero_qualified');
  }

  // Low match count (1–2 vendors) → still send to those vendors, but ALSO flag for admin review
  const isLowMatch = qualified.length <= 2;
  if (isLowMatch) {
    console.warn('[AUTO-MATCH] ⚠️ Low match count:', qualified.length, '— flagging for admin review');
    await _flagLowMatchForAdmin(rfqId, rfqInfo.categorySlug, rfqInfo.county, qualified.length);
  }
