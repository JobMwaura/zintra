      ...
      try {
        ...
        if (typeof url === 'string' && url.includes('amazonaws.com')) {
          ...
          try {
            ...
            console.log('🔄 Regenerating vendor image from S3 URL (extracted key)');
            const freshUrl = await generateFileAccessUrl(extractedKey);
            regeneratedUrls[url] = freshUrl;
            console.log('✅ Successfully regenerated vendor image URL from S3 URL');
          } catch (parseError) {
            console.warn('⚠️ Failed to extract key from vendor image S3 URL, keeping original:', parseError.message);
